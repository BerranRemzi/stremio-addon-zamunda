...
async function testZamundaSE() {
    console.log('=== Testing Zamunda.se API ===\n');
    ...
    try {
        ...
        const loginSuccess = await zamundaSE.login();
        ...
        const searchResults = await zamundaSE.search('Avatar');
        ...
        const movieResults = await zamundaSE.searchByTitle('The Matrix', 1999);
        ...
        if (movieResults.length > 0) {
            ...
            const streams = await zamundaSE.formatTorrentsAsStreams(movieResults.slice(0, 2));
