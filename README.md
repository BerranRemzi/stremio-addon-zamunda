...
A Stremio addon that enables streaming torrents from Zamunda.net directly in your Stremio client. This addon provides seamless integration with Zamunda's torrent tracker, allowing you to search and stream movies and TV series with Bulgarian audio/subtitles support.

## ✨ Features

### 🎬 Content Streaming
- **Movie Streaming**: Full support for movies with quality detection and metadata
- **Series Streaming**: Complete TV series support with advanced episode/season handling
  - Single episode torrents (S01E05, 1x05, etc.)
  - Season packs with automatic episode file selection
  - Multi-episode ranges (S01E01-E05)
  - Episode-specific search and filtering

### 🔍 Smart Search & Matching
- **Intelligent Title Matching**: Fuzzy matching with year filtering
- **Multi-Format Support**: Handles various naming conventions (S01E05, 1x05, Season 1 Episode 5)
- **Bulgarian Content Detection**: Automatic detection of Bulgarian audio and subtitles (🇧🇬)
- **Resolution Detection**: Auto-detects quality (480p, 576p, 720p, 1080p, 1440p, 4K, 8K, 3D)

### 🚀 Performance & Reliability
- **In-Memory Caching**: Fast torrent file caching (50 file limit, LRU eviction)
- **OMDb Integration**: Enhanced title resolution with OMDb API
- **Session Management**: Automatic Zamunda login and session handling
- **Bounded Parallelism**: Concurrent torrent processing (max 3 simultaneous)
- **Error Resilience**: Robust error handling with graceful fallback
- **Timeout Protection**: 10-15 second timeouts on all network requests

### 📊 Rich Metadata Display
- **Seeders/Leechers Count**: Real-time availability info (👤)
- **File Sizes**: Accurate size information (💾)
- **Quality Indicators**: Clear resolution labels
- **Bulgarian Audio Flags**: Visual indicator for BG content (🇧🇬)

### 🌐 Deployment Options
- **Serverless Support**: Ready for Vercel/Now deployment
- **Local Hosting**: Run on your own server/computer
...
- **`zamunda-movie-parser.js`**: HTML parsing and torrent processing for movies
- **`zamunda-series-parser.js`**: HTML parsing and torrent processing for TV series
- **`torrentFileManager.js`**: In-memory torrent file caching
- **`serverless.js`**: Vercel serverless function handler

### Data Flow
1. Stremio requests streams via IMDb ID (with season/episode for series)
2. Addon fetches metadata from OMDb API (cached)
3. Zamunda API searches for matching torrents (movies or series)
4. For series: filters results by season/episode, supports season packs
5. Torrent files are downloaded and cached
6. Streams are formatted and returned to Stremio
...
├── zamunda-movie-parser.js # HTML parsing for movies
├── zamunda-series-parser.js # HTML parsing for series
